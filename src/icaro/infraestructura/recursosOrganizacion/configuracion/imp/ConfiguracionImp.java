...
public class ConfiguracionImp extends ClaseGeneradoraConfiguracion {
	...
	private String descXML = NombresPredefinidos.RUTA_DESCRIPCIONES + identFicheroDescripcion
			+ ".xml";
		...
		try {
			...
			try {
				...
				NombresPredefinidos.ACTIVACION_PANEL_TRAZAS_DEBUG = Boolean
						.toString(this.descOrganizacion.getPropiedadesGlobales()
			...
			try {
				...
				if (comprobador.comprobar(descOrganizacion)) {
					try {
						...
						java.util.logging.Logger.getLogger(ConfiguracionImp.class.getName()).log(
								Level.SEVERE, " --Error al obtener el identificador del Host  -- ",
								ex);
					}
					...
					properties = ConstructorProperties.obtenerProperties(descOrganizacion
							.getPropiedadesGlobales().getListaPropiedades());
					properties.setProperty(
							NombresPredefinidos.INTERVALO_MONITORIZACION_ATR_PROPERTY, Integer
									.toString(descOrganizacion.getPropiedadesGlobales()
											.getIntervaloMonitorizacionGestores()));
					properties.setProperty(NombresPredefinidos.ACTIVACION_PANEL_TRAZAS_DEBUG,
							Boolean.toString(descOrganizacion.getPropiedadesGlobales()
									.isActivarPanelTrazasDebug()));
				} else {
					trazas.trazar(
							this.id,
							"\n\nError al comprobar los comportamientos de los gestores, agentes y recursos descritos en el fichero de descripcion del XML "
									+ xmlDescripcion.getAbsolutePath(), InfoTraza.NivelTraza.error);
				}
			} catch (Exception e) {
				logger.fatal(
						"\n\nError al comprobar los comportamientos de los gestores, agentes y recursos descritos en el fichero de descripcion del XML "
		...
		try {
			...
			if (comprobador.comprobar(descOrganizacion)) {
				try {
					...
					java.util.logging.Logger.getLogger(ConfiguracionImp.class.getName()).log(
							Level.SEVERE, " --Error al obtener el identificador del Host  -- ", ex);
				}
				constructorDescInstancias = new ConstructorDescOrganizacion(descOrganizacion);
				...
				properties.setProperty(NombresPredefinidos.INTERVALO_MONITORIZACION_ATR_PROPERTY,
						Integer.toString(descOrganizacion.getPropiedadesGlobales()
								.getIntervaloMonitorizacionGestores()));
				properties.setProperty(NombresPredefinidos.ACTIVACION_PANEL_TRAZAS_DEBUG, Boolean
						.toString(descOrganizacion.getPropiedadesGlobales()
								.isActivarPanelTrazasDebug()));
				return true;
			} else {
	...
	@Override
	public String getHostComponente(String idComponente) throws ExcepcionEnComponente,
			RemoteException {
	...
	public String getHostRecurso(String idRecurso) throws ExcepcionEnComponente, RemoteException {
		...
		if (descRecurso != null) {
			...
			logger.fatal("\n\nError La descripcion del  comportamiento del recurso  " + idRecurso
	...
	@Override
	public int getValorNumericoPropiedadGlobal(String atributo) throws ExcepcionEnComponente,
			RemoteException {
	...
	public String getIdentGestorInicial() {
		...
		return descOrganizacion.getDescInstancias().getGestores().getInstanciaGestor().get(0)
				.getId();
	}
	...
	@Override
	public Boolean esComponenteRemoto(String idRecurso) throws ExcepcionEnComponente,
			RemoteException {
