 ...
 */
public class ProcesadorInfoReactivoAutInObjImp0 extends ProcesadorInfoReactivoAbstracto implements
		Serializable {
	...
	public ProcesadorInfoReactivoAutInObjImp0(ItfUsoAutomataEFE interpAutom,
			GestorAccionesAgteReactivoImp gestAcciones, AgenteReactivoAbstracto implItfsagente)
	...
	public synchronized void procesarInfoControlAgteReactivo(Object infoParaProcesar) {
		NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte, "Se procesa el  input "
				+ infoParaProcesar, InfoTraza.NivelTraza.debug);
		if (this.estado == InterfazGestion.ESTADO_ACTIVO)
			if (infoParaProcesar instanceof InfoContEvtMsgAgteReactivo) {
				...
				resultadoProcesoInput = automataControl.procesaInput(infoParaAutomata.getInput(),
	...
	public synchronized void procesarInput(Object input, Object... paramsAccion) {
		NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte, "Se procesa el  input " + input,
				InfoTraza.NivelTraza.debug);
		...
		if (input == null) {
			NombresPredefinidos.RECURSO_TRAZAS_OBJ.aceptaNuevaTraza(new InfoTraza(this.getClass()
					.getSimpleName(), "ERROR: El input a procesar no puede ser null ",
	...
	public void inicializarInfoGestorAcciones(String identAgte,
		...
		if (gestorAcciones != null) {
			...
			gestorAcciones
					.inicializarInfoAccionesAgteReactivo(identAgte, percepcionProductor, this);
		}
	}
