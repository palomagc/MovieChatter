...
public class ProcesadorItemsPercepReactivo {
	...
	public ProcesadorItemsPercepReactivo(AgenteReactivoAbstracto agente,
		...
		try {
			...
			java.util.logging.Logger.getLogger(ProcesadorItemsPercepReactivo.class.getName()).log(
	...
	private boolean filtrarItem() {
		if (item instanceof EventoSimple) {
			...
			if (!mensaje.getReceptor().equals(nombreAgente)) {
				log.warn("El mensaje" + mensaje.toString() + " no tiene como receptor el agente "
						+ nombreAgente + "\n Destinatario del mensaje: " + mensaje.getReceptor());
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Percepcion: El mensaje "
						+ mensaje.toString() + " no tiene como receptor el agente " + nombreAgente
						+ "\n Destinatario del mensaje: " + mensaje.getReceptor(),
						InfoTraza.NivelTraza.debug));
				return false;
			} else
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Percepcin: Item " + item
					+ " no reconocido", InfoTraza.NivelTraza.debug));
			return false;
		}
	}
	...
	private InfoContEvtMsgAgteReactivo extraerInfoControl() {
		...
		if (item instanceof EventoSimple) {
			...
			log.error("Item " + item + " no reconocido");
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Percepcion: Item " + item
					+ " no reconocido", InfoTraza.NivelTraza.debug));
		}
	...
	public synchronized void procesarItem(Object item) throws RemoteException {
		...
		if (item instanceof EventoInternoAgteReactivo)
			itfControlReactivo.procesarInput(((EventoInternoAgteReactivo) item).input,
			...
			if (infoExtraida != null)
				...
				log.error("No se ha podido extraer informacion valida del  " + item
						+ " no reconocido");
				trazas.aceptaNuevaTraza(new InfoTraza(this.agente.getIdentAgente(),
						"Percepcion: Item " + item + " no se ha podido extraer informacion valida",
						InfoTraza.NivelTraza.debug));
			}
		}
	}
