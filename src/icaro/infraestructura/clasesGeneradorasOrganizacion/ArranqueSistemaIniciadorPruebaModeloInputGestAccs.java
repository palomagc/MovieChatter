...
public class ArranqueSistemaIniciadorPruebaModeloInputGestAccs {
	...
	public static void main(String args[]) {
		...
		try {
			...
			repositorioInterfaces.registrarInterfaz(NombresPredefinidos.ITF_GESTION
					+ NombresPredefinidos.RECURSO_TRAZAS, recursoTrazas);
		...
		//
		// ItfGestIniciador = (ItfGestionAgenteReactivo)
		...
		// ItfUsoIniciador = (ItfUsoAgenteReactivo)
		// ClaseGeneradoraRepositorioInterfaces.instance().obtenerInterfaz(
		...
		try {
			...
			FactoriaComponenteIcaro.instanceAgteReactInpObj().crearAgenteReactivo(identPropietario,
					rutaCarpetaAcciones);
			...
			// "Antes de enviar el evento Estoy en el estado : "+ estadoActual, NivelTraza.debug);

			itfUsoIniciador.aceptaEvento(eventoPrueba);
			// estadoActual =itfUsoIniciador.getItfControl().getEstadoControlAgenteReactivo();
			// recursoTrazas.trazar(identPropietario,
			// "Despues de enviar el evento Estoy en el estado : "+ estadoActual, NivelTraza.debug);
			...
			recursoTrazas.trazar(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION, msgUsuario,
					NivelTraza.error);
