...
public abstract class TareaSincrona {
	...
	public void generarInformeConCausaTerminacion(String idTarea, Objetivo contxtGoal,
			String idAgenteOrdenante, Object contenido, CausaTerminacionTarea causaTerminacion) {

		String identGoal = null;
		if (contxtGoal != null)
			identGoal = contxtGoal.getgoalId();
		InformeDeTarea resultadoTarea = new InformeDeTarea(idTarea, identGoal, idAgenteOrdenante,
				contenido, causaTerminacion);
	...
	public void generarInformeOK(String idTarea, Objetivo contxtGoal, String idAgenteOrdenante,
			Object contenido) {
		String goalId = null;
		if (contxtGoal != null) {
			goalId = contxtGoal.getgoalId();
		}
		InformeDeTarea resultadoTarea = new InformeDeTarea(idTarea, goalId, idAgenteOrdenante,
				contenido);
		itfProcObjetivos.insertarHecho(resultadoTarea);
		// envioHechos.insertarHecho(contenido);
	}

	public void generarInformeTemporizado(long milis, String idTarea, Objetivo contxtGoal,
		...
		InformeDeTarea informeTarea = new InformeDeTarea(idTarea, goalId, idAgenteOrdenante,
				contenido);
		InformeTimeout informeTemporizado = new InformeTimeout(milis, itfProcObjetivos,
				informeTarea);
		informeTemporizado.start();
	}

	public void generarInformeTemporizadoFromConfigProperty(String identproperty,
			Objetivo contxtGoal, String idAgenteOrdenante, Object contenido) {
		try {
			// el nombre de la propiedad es el nombre de la tarea y debe estar definido en la
			// configuracion. El valor debe ser un entero
			// identproperty = NombresPredefinidos.PREFIJO_PROPIEDAD_TAREA_TIMEOUT+identproperty;
			int valorTimeout = this.getItfUsoConfiguracion().getValorNumericoPropiedadGlobal(
			...
			if (valorTimeout <= 0) {
				...
				valorTimeout = this.getItfUsoConfiguracion().getValorNumericoPropiedadGlobal(
				...
				contenido = NombresPredefinidos.PREFIJO_MSG_TIMEOUT;
			this.generarInformeTemporizado(valorTimeout, identproperty, contxtGoal,
