...
public class InterpreteAutomataEFconGestAcciones implements ItfUsoAutomataEFE, Serializable {
	 ...
	 *            en esta clase)
	 */
	public InterpreteAutomataEFconGestAcciones(TablaEstadosAutomataEFinputObjts intpTablaEstados,
		...
		if (NombresPredefinidos.RECURSO_TRAZAS_OBJ == null) {
			try {
				...
				java.util.logging.Logger.getLogger(
						InterpreteAutomataEFconGestAcciones.class.getName()).log(Level.SEVERE,
						null, ex);
			}
		} else
	...
	public synchronized boolean ejecutarTransicion(String input) {
		...
		if (!tablaEstadosAutomata.esInputValidoDeEstado(estadoActual, input)) {
			this.trazas.trazar(this.getClass().getSimpleName(), " El input : " + input
					+ " No coincide con los inputs definidos en el automata " + " en el estado  :"
					+ estadoActual, InfoTraza.NivelTraza.error);
			return false;
		}
		...
		if (transicion == null) {
			...
			this.trazas.trazar(this.getClass().getSimpleName(),
					" No existe transicion asociada al input : " + input + " en el estado  :"
							+ estadoActual, InfoTraza.NivelTraza.error);
			return true;
		}
		// obtenemos el tipo de transción ************* Revisar
	...
	private synchronized boolean ejecutarTransicion(String input, Object[] params) {
		...
		String estado = estadoActual();
		TransicionAutomataEF transicion = this.tablaEstadosAutomata.getTransicion(estadoActual()
				+ input);
		if (transicion == null) {
			// mensaje de error el input no es valido en el estado actual
			this.trazas.trazar(this.getClass().getSimpleName(),
					" No existe transicion asociada al input : " + input + " en el estado  :"
							+ estadoActual, InfoTraza.NivelTraza.debug);
			return false;
		}
		this.cambiaEstado(transicion.getidentEstadoSiguiente());
		Integer tipoTransicion = transicion.getTipoTransicion();
		...
		try { // se trata de una claseAcciones semanticas donde no se ha
			...
			java.util.logging.Logger.getLogger(InterpreteAutomataEFconGestAcciones.class.getName())
