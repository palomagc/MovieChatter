...
public class ComunicacionAgentes {
		...
		try {
			...
			if (itfUsoAgente == null) { // busco la interfaz en los repositorios
				...
				if (itfUsoAgente == null)
					itfUsoAgente = (InterfazUsoAgente) AdaptadorRegRMI.getItfAgenteRemoto(
							nombreAgente, NombresPredefinidos.ITF_USO);
			}
			if (itfUsoAgente == null)
				return false;
			else {
	...
	public synchronized boolean enviarInfoAotroAgente(Object infoAEnviar, String identAgteReceptor) {
		try {
			if (existItfAgte(identAgteReceptor)) {
				MensajeSimple mensajeAenviar = new MensajeSimple(infoAEnviar, agentePropietario,
						identAgteReceptor);
				...
				logger.error("El agente: " + agentePropietario
						...
						+ identAgteReceptor + ".No se encontro la interfaz del agente Receptor");
				trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
	...
	public synchronized boolean enviarInfoConMomentoCreacionAotroAgente(Object infoAEnviar,
			String identAgteReceptor) {

		return (enviarMsgaOtroAgente(new MensajeSimpleConInfoTemporal(infoAEnviar,
		...
		try {
			if (existItfAgte((String) mensajeAenviar.getReceptor())) {
				...
				logger.error("El agente: " + agentePropietario
						...
						+ ".No se encontro la interfaz del agente Receptor");
				trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
						"No se pudo enviar la informacion : " + mensajeAenviar
								+ ". No se encontro la interfaz del agente Receptor",
						InfoTraza.NivelTraza.error));
				return false;
			}
		} catch (Exception e) {
			logger.error("El agente: " + agentePropietario + "-- no pudo   enviar el mensaje  : "
					+ mensajeAenviar + ".No se encontro la interfaz del agente Receptor");
			trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
					"No se pudo enviar la informacion : " + mensajeAenviar
							+ ". No se encontro la interfaz del agente Receptor",
					InfoTraza.NivelTraza.error));
	...
	public synchronized void informaraOtroAgenteReactivo(InfoContEvtMsgAgteReactivo infoAEnviar,
			String identAgenteReceptor, ItfUsoAgenteReactivo itfUsoAgenteReceptor) {
		...
		try {
			if (itfUsoAgenteReceptor != null) {
				...
				logger.error("El agente: " + agentePropietario
						+ "-- no puedo   enviar la informacion : " + infoAEnviar + " Al agente: "
						+ identAgenteReceptor + " La interfaz del agente Receptor es null ");
	...
	public synchronized void informaraOtroAgenteReactivo(InfoContEvtMsgAgteReactivo infoAEnviar,
		...
		try {
			if (existItfAgte(identAgenteReceptor)) {
				...
				logger.error("El agente: " + agentePropietario
						+ "-- no puedo   enviar la informacion : " + infoAEnviar + " Al agente: "
						+ identAgenteReceptor + ".No se encontro la interfaz del agente Receptor");
				trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
						"No se pudo enviar la informacion : " + infoAEnviar + " Al agente: "
								+ identAgenteReceptor
								+ ". No se encontro la interfaz del agente Receptor",
	...
	public synchronized void informaraGrupoAgentes(Object infoAEnviar,
		...
		enviarMsgaGrupoAgentes(new MensajeSimple(infoAEnviar, this.agentePropietario, null),
				identsAgteReceptores);
	}

	public synchronized void informarConMomentoCreacionaGrupoAgentes(Object infoAEnviar,
			ArrayList<String> identsAgteReceptores) {
	...
	public synchronized void enviarMsgaGrupoAgentes(MensajeSimple msgAEnviar,
		...
		try {
			...
			logger.error("El agente: " + agentePropietario + "-- no pudo   enviar el mensaje : "
					+ msgAEnviar);
			trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
					"No se pudo enviar la informacion : " + msgAEnviar, InfoTraza.NivelTraza.error));
		}
	}
