...
public class ProcesadorItems implements ItfProcesadorItems {
	...
	public ProcesadorItems(AgenteCognitivo agente) {
		this.agente = agente;
		this.infoExtractedQ = new LinkedBlockingQueue<ExtractedInfo>(CAPACIDAD_BUZON_CONT_MESSG);
	...
	private boolean filtrarItem() {
		...
		if (item instanceof EventoSimple) {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(this.agente.getIdentAgente(), "Percepcion: Item "
					+ item + " no reconocido", InfoTraza.NivelTraza.debug));
			return false;
		}
	}
				...
				if (filtradoPercepcion == false) {
					...
					try {
						...
						if (infoExtr != null) {
							...
							trazas.aceptaNuevaTraza(new InfoTraza(agente.getIdentAgente(),
									"Procesador de Items: El contenido del mensaje "
											+ infoExtr.toString() + " se envia al motor "
											+ agente.getIdentAgente(), InfoTraza.NivelTraza.debug));
							if (!seguirEnviando)
