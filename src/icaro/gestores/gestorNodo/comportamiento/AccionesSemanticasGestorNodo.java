 ...
 * @created Enero 2011
 */
public class AccionesSemanticasGestorNodo extends AccionesSemanticasAgenteReactivo implements
		Serializable {
	/**
	 * 
	 */
	...
	public AccionesSemanticasGestorNodo() {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Construyendo agente reactivo "
					+ nombreAgente + ".", InfoTraza.NivelTraza.debug));
			...
			itfUsoPropiadeEsteAgente = (ItfUsoAgenteReactivo) itfUsoRepositorio
					.obtenerInterfazUso(NombresPredefinidos.ITF_USO + nombreAgente);
		} catch (Exception ex) {
	...
	public void comenzar() {
		try { /* Saco mi configuracion */
			...
			Logger.getLogger(AccionesSemanticasGestorNodo.class.getName()).log(Level.SEVERE, null,
					ex);
		...
		if (!AdaptadorRegRMI.addElement2LocalRegRMI("GestorNodo", itfUsoPropiadeEsteAgente)) {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Arranco el RMI en el puerto:"
					+ puertoRMI, InfoTraza.NivelTraza.debug));
	...
	public void configurarGestor() {
		try {
			/*
			 * En esta accion semantica se configura todo aquello que sea necesario a partir del
			 * archivo xml
			 */
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Hubo problemas al configurar el gestor de Nodo.", InfoTraza.NivelTraza.error));
		}
	...
	public void crearAgenteAplicacion(String identAgente) {
		...
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Fue imposible crear el agente : "
					+ identAgente, InfoTraza.NivelTraza.error));
			trazas.trazar(nombreAgente, "Fue imposible crear el agente : " + identAgente,
					NivelTraza.error);
			e.printStackTrace();
		}
		try {
		...
		try {
			...
			if (nodoDestino.equals(esteNodo)) {
				...
				while (!ok) {
					++intentos;
					try {
						((FactoriaAgenteReactivo) ClaseGeneradoraRepositorioInterfaces.instance()
								.obtenerInterfaz(
										NombresPredefinidos.FACTORIA_AGENTE_REACTIVO + nodoDestino))
						...
						logger.error("Error al crear el agente "
								+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES
								+ " en un nodo remoto. Se volvera a intentar en " + intentos
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, identGestor + " Gestor  creado.",
					InfoTraza.NivelTraza.debug));
			...
			((ItfGestionAgenteReactivo) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION + identGestor))
					.setGestorAReportar(nombreAgente);

			logger.debug("GestorNodo: Creando gestor de recursos ...");
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Creando gestor de  ..."
					+ identGestor, InfoTraza.NivelTraza.debug));
	...
	public void crearGestores() {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Creando gestor de agentes ...",
					InfoTraza.NivelTraza.debug));
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Creando gestor de recursos ...", InfoTraza.NivelTraza.debug));
			...
			// indico a qui�n debe reportar
			((ItfGestionAgenteReactivo) ClaseGeneradoraRepositorioInterfaces.instance()
					.obtenerInterfaz(
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Gestores registrados correctamente.", InfoTraza.NivelTraza.debug));
	...
	public void arrancarGestorAgentes() {
		...
		try {
			((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES)).arranca();
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Fue imposible arrancar el Gestor de Agentes.", InfoTraza.NivelTraza.error));
			e.printStackTrace();
			try {
	...
	public void arrancarGestorRecursos() {
		...
		try {
			((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS)).arranca();
			// this.itfUsoAgente.aceptaEvento(new
			// EventoRecAgte("gestor_recursos_arrancado_ok"));
		} catch (Exception e) {
			logger.error("GestorOrganizaci�n: Fue imposible arrancar el Gestor de Recursos.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Fue imposible arrancar el Gestor de Recursos.", InfoTraza.NivelTraza.error));
			e.printStackTrace();
			try {
	...
	public void gestoresArrancadosConExito() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
				"Gestor de la organizacion esperando peticiones.", InfoTraza.NivelTraza.debug));
	}

	/**
	 * Decide qu� hacer en caso de fallos en el gestor de agentes y/o en el gestor de recursos
	 */
	public void decidirTratamientoErrorIrrecuperable() {
		 ...
		 * NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION,
		 * NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)); } catch (Exception e) {
		 * e.printStackTrace(); }
		 */
	...
	public void monitorizarGestores() {
		...
		try {
			monitAgentes = ((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES)).obtenerEstado();
			monitRecursos = ((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS)).obtenerEstado();
			...
			if (errorAlMonitorizar) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
						"Error al monitorizar gestores", InfoTraza.NivelTraza.debug));
	...
	public void terminarGestorAgentes() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion", "Terminando gestor de agentes",
				InfoTraza.NivelTraza.debug));
		...
		try {
			...
			if (gestorAgentes != null) {
				...
				this.generarTimeOut(2000, "timeout_gestor_agentes", nombreAgente, nombreAgente);
			} else
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"No se pudo acceder al gestor de agentes.", InfoTraza.NivelTraza.debug));
			ex.printStackTrace();
		}
	}
	...
	public void terminarGestorRecursos() {
		...
		try {
			...
			if (gestorRecursos != null) {
				...
				// timeout de 5 segundos
				this.generarTimeOut(2000, "timeout_gestor_recursos", nombreAgente, nombreAgente);
			} else
				this.informaraMiAutomata("continuaTerminacion");
			// this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
			// "continuaTerminacion",
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"No se pudo acceder al gestor de recursos.", InfoTraza.NivelTraza.debug));
			ex.printStackTrace();
		}
	...
	private void crearUnAgenteReactivo(String identAgenteAcrear) throws Exception {
		try {
			...
			if (config == null)
				config = (ItfUsoConfiguracion) ClaseGeneradoraRepositorioInterfaces.instance()
						.obtenerInterfaz(
			...
			if (nodoDestino.equalsIgnoreCase(esteNodo)) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorNodo", "El nodo especificado:"
						+ nodoDestino + " para el agente " + identAgenteAcrear
						+ " no se corresponde con este nodo  " + esteNodo,
						InfoTraza.NivelTraza.error));

				logger.error("El nodo especificado:" + nodoDestino + " para el agente "
						+ identAgenteAcrear + " no se corresponde con este nodo  " + esteNodo);
			...
			logger.error(
					"GestorNodo: Error al crear el agente reactivo " + identAgenteAcrear + ".", ex);
	...
	private void crearUnAgenteCognitivo(String nombreAgente) throws Exception {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Construyendo agente Cognitivo "
					+ nombreAgente + ".", InfoTraza.NivelTraza.debug));
					...
					try {
						((FactoriaAgenteCognitivo) this.itfUsoRepositorio
								.obtenerInterfaz(NombresPredefinidos.FACTORIA_AGENTE_COGNITIVO
										+ nodoDestino)).crearAgenteCognitivo(instancia);
						ok = true;
					} catch (Exception e) {
			...
			logger.error("GestorAgentes: Error al crear el agente Cognitivo " + nombreAgente + ".",
					ex);
	...
	public void crearUnRecursoEnNodoLocal(String identInstRecurso) throws Exception {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Construyendo recurso "
					+ identInstRecurso + ".", InfoTraza.NivelTraza.debug));
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Recurso "
					+ descRecurso.getId() + " creado.", InfoTraza.NivelTraza.debug));

		} catch (Exception ex) {
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Error al crear el recurso "
					+ identInstRecurso + ".", InfoTraza.NivelTraza.error));
	...
	public void crearUnRecursoEnNodoLocalyPublicIntfEnRMIO(String identInstRecurso)
			throws Exception {
		...
		try {
			...
			if (!addEntityIntfs2Local_RMIregistry(identInstRecurso)) {
				...
				// Se informa al Gestor de Recursos de la organizacion de la creacion y registro del
				// recurso
				...
				ItfUsoAgenteReactivo itfUsoAgteReceptor = (ItfUsoAgenteReactivo) AdaptadorRegRMI
						.getItfAgenteRemoto(NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
								NombresPredefinidos.ITF_USO);
				if (itfUsoAgteReceptor != null) {
					comunicator.informaraOtroAgenteReactivo(new InfoContEvtMsgAgteReactivo(
							"RecursoRemotoCreadoyRegistrado"),
							NombresPredefinidos.NOMBRE_GESTOR_RECURSOS, itfUsoAgteReceptor);
				} else {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Error al crear el recurso "
					+ identInstRecurso + ".", InfoTraza.NivelTraza.error));
	...
	public void crearUnAgenteEnNodoLocalyPublicIntfEnRMIO(String identAgenteAcrear)
			throws Exception {
		...
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Construyendo agente "
					+ identAgenteAcrear + ".", InfoTraza.NivelTraza.debug));
			// Recurso de aplicacion: local o remoto?
			...
			if (!nodoDestino.equalsIgnoreCase(esteNodo)) {
				...
				if (tipoAgenteAcrear != TipoAgente.COGNITIVO) { // no es ni cognitivo ni reactivo
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "El subtipo del agente "
							+ identAgenteAcrear + " definido en la configuracion no es correcto.",
							InfoTraza.NivelTraza.error));
				...
				if (!addEntityIntfs2Local_RMIregistry(identAgenteAcrear)) {
					...
					// Se informa al Gestor de Agentes de la organizacion de la creacion y registro
					// del recurso
					// config.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_RECURSOS).getNodo().getNombreUso();
					ItfUsoAgenteReactivo itfUsoAgteReceptor = (ItfUsoAgenteReactivo) AdaptadorRegRMI
							.getItfAgenteRemoto(NombresPredefinidos.NOMBRE_GESTOR_AGENTES,
									NombresPredefinidos.ITF_USO);
					if (itfUsoAgteReceptor != null) {
						// Construimos los parametros que se envian con el input
						Object[] param = { identAgenteAcrear };
						comunicator.informaraOtroAgenteReactivo(new InfoContEvtMsgAgteReactivo(
								"agenteRemotoCreadoyRegistrado", param),
								NombresPredefinidos.NOMBRE_GESTOR_AGENTES, itfUsoAgteReceptor);
					} else {
						trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
								"No he podido obtener la interfaz   del agente: "
										+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES,
					...
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Agente "
							+ identAgenteAcrear + " creado.", InfoTraza.NivelTraza.debug));
				}
			}
		} catch (Exception ex) {
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Error al crear el agente "
					+ identAgenteAcrear + ".", InfoTraza.NivelTraza.error));
	...
	public Boolean addEntityIntfs2RMI_ORGregistry(String identEntity) {
		...
		try {
			...
			if (itfEntity != null)
				if (AdaptadorRegRMI.addElementRegRMIOrg(NombresPredefinidos.ITF_USO + identEntity,
						itfEntity)) {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Se añaden las Itfs de uso y de gestion de la entidad :" + identEntity
			...
			itfEntity = (Remote) itfUsoRepositorio.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
					+ identEntity);
			if (itfEntity != null)
				if (AdaptadorRegRMI.addElement2LocalRegRMI(NombresPredefinidos.ITF_GESTION
						+ identEntity, itfEntity)) {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Se añaden las Itfs de uso y de gestion de la entidad :" + identEntity
									+ " -- al RMI registry de la organizacion",
							InfoTraza.NivelTraza.debug));
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Error al registrar  la entidad "
					+ identEntity + ". en el registro RMI de la organizacion",
					InfoTraza.NivelTraza.error));
			logger.error("GestorRecursos: Error al crear el recurso " + identEntity
					+ ".en el registro RMI de la organizacion", ex);
			informaraMiAutomata("error_en_registroRemoto_recurso");
			// throw ex;
		}
		return interfacesRegistradas;
	...
	public Boolean addEntityIntfs2Local_RMIregistry(String identEntity) {
		...
		try {
			Remote itfEntity = (Remote) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + identEntity);
			if (itfEntity != null)
				if (AdaptadorRegRMI.addElement2LocalRegRMI(NombresPredefinidos.ITF_USO
						+ identEntity, itfEntity)) {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Se añaden las Itfs de uso y de gestion de la entidad :" + identEntity
									+ " -- al RMI registry local", InfoTraza.NivelTraza.debug));
				} else {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"No he podido añadir la entidad :" + identEntity
									+ " -- al RMI registry local.", InfoTraza.NivelTraza.error));
					informaraMiAutomata("error_en_registroRemoto_recurso");
					return interfacesRegistradas;
				}
			else {
	...
	public void arrancarRecurso(String identRecurso) {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Arrancando recurso :" + identRecurso,
				InfoTraza.NivelTraza.debug));
		...
		try {
			...
			logger.debug("GestorRecursos: Es necesario arrancar el recurso " + identRecurso
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Es necesario arrancar el recurso " + identRecurso
							+ ", recuperando interfaz de gesti�n.", InfoTraza.NivelTraza.debug));
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Arrancando el recurso "
					+ identRecurso + ".", InfoTraza.NivelTraza.debug));
			if (itfGesAg != null) {
				itfGesAg.arranca();
				...
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						" No se puede dar la orden de arranque al recurso " + identRecurso
								+ ". Porque su interfaz es null", InfoTraza.NivelTraza.debug));
		...
		if (errorEnArranque) { // ha ocurrido alg�n problema en el arranque del
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Gestor de recursos esperando peticiones.", InfoTraza.NivelTraza.debug));
	...
	public void procesarPeticionTerminacion() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
				"Procesando la peticion de terminacion", InfoTraza.NivelTraza.debug));
		...
		/*
		 * try { // this.itfUsoAgente.aceptaEvento(new // EventoRecAgte("termina",null,null));
		 * 
		 * 
		 * ItfGestionAgenteReactivo gestion = (ItfGestionAgenteReactivo) this.itfUsoRepositorio
		 * .obtenerInterfaz(NombresPredefinidos.ITF_GESTION +
		 * NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION); gestion.termina(); } catch (Exception e)
		 * { e.printStackTrace(); }
		 */
	}
	...
	public void comenzarTerminacionConfirmada() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
				"Comenzando la terminacion de la organizacion...", InfoTraza.NivelTraza.info));
		try {
			// String estadoInternoAgente = this.ctrlGlobalAgenteReactivo.getItfControl()
			// .getEstadoControlAgenteReactivo();
			// ItfGestionAgenteReactivo gestion = (ItfGestionAgenteReactivo) this.itfUsoRepositorio
			// .obtenerInterfaz(NombresPredefinidos.ITF_GESTION + this.nombreAgente);
			// gestion.termina();
			...
			this.ctrlGlobalAgenteReactivo.getItfControl().procesarInfoControlAgteReactivo(
	...
	/**
	 * Da orden de terminaci�n al gestor de agentes y/o al gestor de recursos si est�n activos.
	 */
	/*
	 * public void terminarGestoresActivos() { // mandamos la orden de terminar a los gestores
	 * logger.debug("GestorOrganizaci�n: Terminando los gestores"); try {
	 * ((InterfazGestion)this.itfUsoRepositorio
	 * .obtenerInterfaz(NombresInterfaces.ITF_GES_GESTOR_AGENTES)).termina(); } catch (Exception ex)
	 * { logger.debug("GestorOrganizaci�n: No se pudo acceder al gestor de agentes.");
	 * ex.printStackTrace(); } try {
	 * ((InterfazGestion)this.itfUsoRepositorio.obtenerInterfaz(NombresInterfaces
	 * .ITF_GES_GESTOR_RECURSOS)).termina(); } catch (Exception ex) {
	 * logger.debug("GestorOrganizaci�n: No se pudo acceder al gestor de recursos.");
	 * ex.printStackTrace(); } try { this.itfUsoAgente.aceptaEvento(new
	 * EventoRecAgte("gestores_terminados",null,null)); } catch (Exception e) { e.printStackTrace();
	 * } }
	 */
	/**
	 * Intenta recuperar los errores detectados en la monitorizaci�n siguiendo la pol�tica
	 * definida para cada gestor.
	 */
	public void recuperarErrorAlMonitorizarGestores() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
				"No se pudo recuperar el error de monitorizaci�n.", InfoTraza.NivelTraza.debug));
		try {
			this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
	...
	public void terminarGestorOrganizacion() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Cerrando sistema.",
				InfoTraza.NivelTraza.debug));
		...
		System.exit(0);
		/*
		 * if (ORBDaemonExec.finalInstance() != null) { ORBDaemonExec.finalInstance().finalize(); }
		 */
	}
