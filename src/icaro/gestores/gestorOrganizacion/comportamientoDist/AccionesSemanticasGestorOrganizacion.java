...
public class AccionesSemanticasGestorOrganizacion extends AccionesSemanticasAgenteReactivo {
	...
	public AccionesSemanticasGestorOrganizacion() {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Fue imposible crear el repositorio de interfaces.", InfoTraza.NivelTraza.error));
			ex.printStackTrace();
			System.exit(1);
		}
	}
	...
	public void configurarGestor() {
		try {
			/*
			 * En esta acci�n sem�ntica se configura todo aquello que sea necesario a partir del
			 * archivo xml
			 */
			...
			this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte("gestor_configurado",
	...
	public void crearGestoreEnEsteNodo(String identAgente) throws RemoteException {
		...
		try {
			...
			if (nodoDestino.equals(identEsteNodo)) {
				...
				logger.error("Error al crear agente " + NombresPredefinidos.NOMBRE_GESTOR_RECURSOS
						+ " en un nodo remoto. Se volver� a intentar en " + " en el nodo local "
						+ identEsteNodo
						+ "\t El nodo del gestor a crear es diferente El Nodo definido es : "
						+ nodoDestino);
				// return false;
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion", "Error al crear agente "
					+ identAgente + " en el nodo local " + identEsteNodo + "\t  ",
					InfoTraza.NivelTraza.error));
			logger.error("Error al crear agente " + identAgente + " en el nodo local "
					+ identEsteNodo + "\t ");
			// return false;
		}
	}
		...
		try {
			...
			while (!ok & (intentos <= maxNumIntentosCreacionCompGestionados)) {
				...
				try {
					...
					trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
							"Error al crear el agente " + NombresPredefinidos.NOMBRE_GESTOR_AGENTES
									+ " en un nodo remoto. Se volver� a intentar en " + intentos
									+ " segundos...\n nodo origen: " + identEsteNodo
									+ "\t nodo destino: " + nodoDestino, InfoTraza.NivelTraza.error));
					logger.error("Error al crear el agente "
							+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES
							+ " en un nodo remoto. Se volver� a intentar en " + intentos
							+ " segundos...\n nodo origen: " + identEsteNodo + "\t nodo destino: "
							+ nodoDestino);
					Thread.sleep(1000 * intentos);
					ok = false;
				}
			...
			if (!ok) {
				trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
						"No se pudo crear el agente " + NombresPredefinidos.NOMBRE_GESTOR_AGENTES
								+ " en un nodo remoto. Se supero el numero de intentos definido "
								+ intentos + identEsteNodo + "\t nodo destino: " + nodoDestino,
						InfoTraza.NivelTraza.error));
				logger.error("Error al crear el agente "
						+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES
						+ " en un nodo remoto. Se supero el numero de intentos definido "
	...
	public void recuperarErrorArranqueGestores() {
		...
		try {
			this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
					"imposible_recuperar_arranque", NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION,
					NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
		} catch (Exception e) {
			e.printStackTrace();
		}
			...
			if (errorAlMonitorizar) {
				...
				if ((monitAgentes == InterfazGestion.ESTADO_ERRONEO_IRRECUPERABLE)
					...
					trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
							...
							InfoTraza.NivelTraza.error));
					this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
							"error_al_monitorizar", NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION,
							NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
				}
			} else {
		...
		try {
			...
			itfGestorAgentes = (InterfazUsoAgente) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
			itfGestorAgentes.aceptaEvento(new EventoRecAgte("ordenTerminacion", nombreAgente,
	...
	public void terminarGestorRecursos() {
		...
		try {
			...
			gestorRecursos = (InterfazUsoAgente) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS);
			gestorRecursos.aceptaEvento(new EventoRecAgte("ordenTerminacion", nombreAgente,
					gestorRecursos.getIdentAgente()));
	...
	public void terminarGestorOrganizacion() {
		...
		try {
			...
			((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)).termina();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		logger.debug("GestorOrganizacion: Cerrando sistema.");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion", "Cerrando sistema.",
				InfoTraza.NivelTraza.debug));
	...
	public void tratarTerminacionNoConfirmada() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(
				...
				InfoTraza.NivelTraza.debug));
		try {
			this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte("continuaTerminacion",
					NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION,
					NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
		} catch (Exception e) {
			e.printStackTrace();
