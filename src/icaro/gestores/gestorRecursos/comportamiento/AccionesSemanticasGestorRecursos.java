...
/**
 * Implementaci�n por defecto de las acciones que se ejecutar�n por parte del gestor de
 * recursos.
 * 
 * @author Felipe Polo
...
public class AccionesSemanticasGestorRecursos extends AccionesSemanticasAgenteReactivo {
	...
	public void listarRecursos() {
		try {
			...
			List<DescInstancia> lista = config.getDescInstanciaGestor(
					NombresPredefinidos.NOMBRE_GESTOR_RECURSOS).getComponentesGestionados();
	...
	public void crearRecurso(ArrayList<DescInstanciaRecursoAplicacion> lista, Integer indice) {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Creando recurso.",
				InfoTraza.NivelTraza.debug));
		int j = 0;
		String nombre;
		boolean error = false;
		...
		while (iterador.hasNext() && (encontrado == false)) {
			...
			if (j == indice.intValue()) {// es el que debemos arrancar
				...
				try {
					...
					trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Añadiendo recurso "
							+ nombre + " a la lista de objetos gestionados.",
							InfoTraza.NivelTraza.debug));
					this.nombresRecursosGestionados.add(nombre);
				} catch (Exception ex) {
					logger.error("GestorRecursos: Hubo problemas al crear el recurso " + nombre
							+ " desde la configuracion. Recuperando errores de creaci�n.");
					trazas.aceptaNuevaTraza(new InfoTraza(
							"GestorRecursos",
							"Hubo problemas al crear el recurso " + nombre
									+ " desde la configuracion. Recuperando errores de creaci�n.",
							InfoTraza.NivelTraza.error));
					ex.printStackTrace();
					error = true;
		...
		try {
			...
			if (error == false && encontrado == true) {
				// parametros = new Object[] { lista, new Integer(indice.intValue() + 1) };
	...
	private void crearUnRecurso(DescInstanciaRecursoAplicacion recurso) throws Exception {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Construyendo recurso "
					+ recurso + ".", InfoTraza.NivelTraza.debug));
			...
			if (nodoDestino.equals(esteNodo)) {
				...
				// Si la organizacion esta desplegada en varios nodos entonces hay que registrarlo
				// en el RMI local
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Recurso " + recurso.getId()
					+ " creado.", InfoTraza.NivelTraza.debug));

		} catch (Exception ex) {
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Error al crear el recurso "
					+ recurso.getId() + ".", InfoTraza.NivelTraza.error));
			logger.error("GestorRecursos: Error al crear el recurso " + recurso + ".", ex);
			throw ex;
		}
	}

	private void crearUnRecursoEnNodoRemotoConRMI(DescInstanciaRecursoAplicacion recurso)
			throws Exception {
		...
		if (!misInterfacesEstanEnElRegistroRMILocal) { // Las añado
			...
			if (!AdaptadorRegRMI.addElement2LocalRegRMI(NombresPredefinidos.ITF_USO + nombreAgente,
					this.itfUsoPropiadeEsteAgente)) { // No se ha podido añadir la interfaz de uso
														// al registro local
				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorRecursos",
		...
		try {
			...
			while (!ok & (intentos <= maxNumIntentosCreacionCompGestionados)) {
				...
				try {
					...
					// Se ordena al Gestor de Nodo que cree al Gestor. Se debe esperar la
					// confirmacion
					...
					// ItfUsoAgenteReactivo itfUsoGestorNodo = (ItfUsoAgenteReactivo)
					...
					if (itfUsoGestorNodo != null) {
						...
						// Registro la interfaz de uso del gestor en el registro RMI de la
						// organizacion para que pueda ser localizado y recibir informacion

						itfUsoGestorNodo.aceptaMensaje(new MensajeSimple(
								new InfoContEvtMsgAgteReactivo("peticion_crearRecurso",
										(Object) recurso.getId()), this.nombreAgente,
								NombresPredefinidos.NOMBRE_GESTOR_NODO));
						ok = true;
					...
					trazas.aceptaNuevaTraza(new InfoTraza(
							...
							"Error al crear el recurso:  "
									...
									+ identNodoRecurso, InfoTraza.NivelTraza.error));
					logger.error("Error al crear el recurso " + identRecurso
			...
			if (!ok) {
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "No se pudo crear el recurso "
						+ identRecurso
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Recurso :" + identRecurso
					+ "creado.", InfoTraza.NivelTraza.debug));
		} catch (Exception e) {
			logger.error("GestorRecursos: Error al crear el recurso " + recurso.getId()
					+ "en el nodo remoto " + identNodoRecurso, e);
			trazas.aceptaNuevaTraza(new InfoTraza(identRecurso, "Error al crear el recurso "
					+ identRecurso + "en el nodo remoto " + identNodoRecurso,
					InfoTraza.NivelTraza.error));
			e.printStackTrace();
			try {
	...
	/**
	 * Funcion que genera el evento para comenzar los reintentos de creacion de un recurso
	 * 
	 * @param recurso
	...
	public void recuperarErrorCreacionRecurso(List<DescInstanciaRecursoAplicacion> lista,
		...
		// maxNumIntentosCreacionCompGestionados,
		// indice };
	 ...
	 * correctamente.
	 */
	public void reintentarCreacionRecurso(List<DescInstanciaRecursoAplicacion> lista,
			DescInstanciaRecursoAplicacion recurso, Integer reintento, Integer indice) {
		...
		if (!fin_reintentos) { // se reintenta la creacion
			logger.debug("GestorRecursos: Reintento de creacion " + reintento + " del recurso "
					+ idRecurso);
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Reintento de creacion "
					+ reintento + " del recurso " + idRecurso, InfoTraza.NivelTraza.debug));
			try {
				...
				logger.debug("GestorRecursos: Añadiendo recurso " + idRecurso
						+ " a la lista de objetos gestionados.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Añadiendo recurso "
						+ idRecurso + " a la lista de objetos gestionados.",
						InfoTraza.NivelTraza.debug));
				this.nombresRecursosGestionados.add(idRecurso);
			} catch (Exception ex) {
				logger.error("GestorRecursos: Hubo problemas al crear el recurso " + idRecurso
						+ " desde la configuraci�n. Recuperando errores de creaci�n.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Hubo problemas al crear el recurso " + idRecurso
								+ " desde la configuracion. Recuperando errores de creaci�n.",
						InfoTraza.NivelTraza.error));
				ex.printStackTrace();
				error = true;
		...
		try {
			...
			if (fin_reintentos) { // ya hay que decidir que se va a hacer una
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Agotados los intentos de crear el recurso " + idRecurso
								+ ". Continuando con la creaci�n.", InfoTraza.NivelTraza.debug));
	...
	public void arrancarRecurso(Integer indice) {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Arrancando recurso.",
				InfoTraza.NivelTraza.debug));
		boolean errorEnArranque = false;

		// seleccionamos el recurso que corresponde
		String nombreRec = (String) this.nombresRecursosGestionados.elementAt(indice.intValue());
		try {
			// recuperamos la interfaz de gestion del recurso
			logger.debug("GestorRecursos: Es necesario arrancar el recurso " + nombreRec
					+ ", recuperando interfaz de gestion.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Es necesario arrancar el recurso " + nombreRec
							+ ", recuperando interfaz de gestion.", InfoTraza.NivelTraza.debug));
			...
			if (itfGesAg == null) {// No esta registrada en el repositirio de interfaces del nodo
									// local

				// String identHostRecurso=
				// config.getDescInstanciaRecursoAplicacion(nombreRec).getNodo().getNombreUso();
				// itfGesAg = AdaptadorRegRMI.getItfGestionEntidadRemota(identHostRecurso,
				// nombreRec);
				itfGesAg = (InterfazGestion) AdaptadorRegRMI.getItfRecursoRemoto(nombreRec,
				...
				if (itfGesAg == null)// la intf de gestion es null El recruso no ha sido registrado
				{
					...
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							" No se puede dar la orden de arranque al recurso " + nombreRec
					...
					this.itfUsoRepositorio.registrarInterfaz(NombresPredefinidos.ITF_GESTION
							+ nombreRec, itfGesAg);
					Remote itfUsoRec = AdaptadorRegRMI.getItfRecursoRemoto(nombreRec,
							NombresPredefinidos.ITF_USO);
					...
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							" Registro la interfaz remota del recurso: " + nombreRec
			...
			// else {
			// // La interfaz del recurso no esta en el repositorio local. Una posibilidad es buscar
			// su itenrfaz remota
			// // registrarla en el repositorio de interfaces local al GR y darle la orden de
			// arrancar
			// // String nodoRecurso =
			...
			// if (itfGesAg != null ){
			// // Registro la interfaz en el repositorio local y doy la orden de arrancar
			// this.itfUsoRepositorio.registrarInterfaz(NombresPredefinidos.ITF_GESTION + nombre,
			// itfGesAg);
			// this.itfUsoRepositorio.registrarInterfaz(NombresPredefinidos.ITF_USO + nombre,
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Hubo un problema al acceder al interfaz remoto mientras se arrancaba el recurso "
							+ nombreRec + " en el gestor de recursos.", InfoTraza.NivelTraza.error));
			ex.printStackTrace();
			errorEnArranque = true;
		}
		if (errorEnArranque) { // ha ocurrido alg�n problema en el arranque del
			...
			if (indice.intValue() == (this.nombresRecursosGestionados.size() - 1)) { // ya
				...
				try {
					...
					// NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
					// NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
					this.comunicator.enviarInfoAotroAgente("gestor_recursos_arrancado_ok",
							NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION);
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				logger.debug("GestorRecursos: Terminado arranque recurso " + nombreRec
						+ ". Arrancando el siguiente recurso.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Terminado arranque recurso " + nombreRec
								+ ". Arrancando el siguiente recurso.", InfoTraza.NivelTraza.debug));
				try {
	 ...
	 */
	/*
	 * private boolean esNecesarioArrancar(String nombreRecurso) { Enumeration enume =
	 * configEspecifica.getListaRecursos().enumerateRecurso(); while (enume.hasMoreElements()) {
	 * Recurso item = (Recurso)enume.nextElement(); if (nombreRecurso.equals(item.getNombre()))
	 * return item.getHayQueArrancarlo(); } logger.error("GestorRecursos: No se encontr� ning�n
	 * recurso con nombre "+nombreRecurso+" dentro de la lista de objetos gestionados."); throw new
	...
	/**
	 * intenta arrancar los recursos que especifique la config y hayan dado problemas.
	 */
	public void recuperarErrorArranqueRecurso() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				...
				InfoTraza.NivelTraza.debug));
		try {
			this.itfUsoGestorAReportar.aceptaEvento(new EventoRecAgte("error_en_arranque_gestores",
					NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
					NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
	...
	public void generarInformeErrorIrrecuperable() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				...
				InfoTraza.NivelTraza.error));
		try {
			this.itfUsoGestorAReportar.aceptaEvento(new EventoRecAgte("peticion_terminar_todo",
					NombresPredefinidos.NOMBRE_GESTOR_RECURSOS, this.itfUsoGestorAReportar
							.getIdentAgente()));
	...
	public void finalizarRecurso() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				"finalizarRecurso():Este metodo no esta implementado", InfoTraza.NivelTraza.debug));
	...
	/**
	 * Monitoriza secuencialmente todos los recursos activos que est�n definidos como necesarios
	 * en la monitorizaci�n.
	 */
	public void monitorizarRecursos() {
		...
		// gestionando
		Enumeration<String> enume = nombresRecursosGestionados.elements();
		while (enume.hasMoreElements() && !errorEncontrado) {
			...
			try {
				...
				if (monitoriz == 0) {
					...
					trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Recurso " + nombre
							+ " esta en estado erroneo o terminado.", InfoTraza.NivelTraza.debug));
				} else
					System.out.println("GestorRecursos:Recurso " + nombre + " est� ok.");

			} catch (Exception ex) {
				errorEncontrado = true;
				logger.error("GestorRecursos: Hubo errores al monitorizar.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Hubo errores al monitorizar.", InfoTraza.NivelTraza.error));
				ex.printStackTrace();
			}
		}
	...
	/**
	 * Da orden de terminaci�n a todos los recursos que se encuentran activos/arrancando
	 */
	public void terminarRecursosActivos() {
		logger.debug("GestorRecursos: Terminando los recursos que estan activos.");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				"Terminando los recursos que estan activos.", InfoTraza.NivelTraza.debug));
		// recorremos todos los recursos gestionados
		Enumeration<String> enumRecursos = this.nombresRecursosGestionados.elements();
		String nombre = "";
		while (enumRecursos.hasMoreElements()) {
			try {
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Terminando el recurso "
						+ nombre + ".", InfoTraza.NivelTraza.debug));
				if (itfGesRec != null) {
					itfGesRec.termina();
	...
	public void terminarGestorRecursos() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Terminando gestor de recursos.",
				InfoTraza.NivelTraza.debug));
		try {
			if (this.hebra != null)
		...
		try {
			((InterfazGestion) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS)).termina();
