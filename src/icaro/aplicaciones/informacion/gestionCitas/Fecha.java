...
/**
 * <p>
 * Title: Cita2 - Sistema Vocal de citas para directivos
 * </p>
 * <p>
 * Description: Clase que representa las fechas en el sistema
 * </p>
 * <p>
 * Copyright: Telef�nica I+D Copyright (c) 2002
 * </p>
 * <p>
 ...
 * @version 3.0
 */
public class Fecha implements Comparable<Object>, Cloneable, java.io.Serializable {
	 ...
	 *            valor de 1 a 31
	 * @param mes
	 *            valor de ENERO a DICIEMBRE
	 * @param anno
	 *            valor de 4 CIFRAS de 2001 en adelante
	 */
	...
	/**
	 * Devuelve el sucesor del objeto fecha actual. El sucesor se calcula como el d�a siguiente a
	 * la fecha actual
	 *
	...
	/**
	 * Determina si la fecha est� completa, es decir, ninguno de sus campos est� indefinido
	 *
	 * @return true si la fecha tiene todos sus campos completos
	 */
	public boolean estaCompleta() {
		if (this.dia != Utilidades.int_INDEFINIDO & this.mes != Utilidades.int_INDEFINIDO
	 ...
	 *         Febrero y dia mayor que 28 2 -> fecha con mes distinto a Febrero y con 30 dias y
	 *         valor para dia mayor que 30
	 */
	public int esFechaValida() {

		int fechaCorrecta = Fecha.FECHA_VALIDA;

		// Si el mes es Febrero, el dia debe ser menor o igual que 28
		if ((this.mes == Fecha.FEBRERO) && (this.dia > 28)) {
			fechaCorrecta = Fecha.ERROR_FEBRERO;
		} else {
			...
			if ((this.mes % 2) == 0 && (this.mes <= 6) && (this.dia > 30))
				fechaCorrecta = Fecha.ERROR_NO_FEBRERO;
			// Caso 4
	...
	/**
	 * Comprueba si la fecha se corresponde con un d�a de fin de semana, esto es, s�bado o
	 * domingo
	 * 
	 * @param fecha
	...
	/**
	 * Comleta la fecha con los campos que no est�n indefinidos de una fecha nueva
	 *
	 * @param fechaNueva
	 *            fecha con la que completar la fecha actual
	 */
	...
	/**
	 * Expresa una instancia de Fecha en lenguaje natural de acuerdo al modo dicho por el usuario
	 *
	 * @return Description of the Return Value
	 */
	public String expresarFechaModoUsuario() {
		...
		if (this.modoUsuario == Fecha.MODO_HOY)
			// El usuario dijo 'hoy'
			sb.append("hoy");
		else if (this.modoUsuario == Fecha.MODO_MANIANA)
			// El usuario dijo 'ma�ana'
			sb.append("ma�ana");
		else if (this.modoUsuario == Fecha.MODO_ABSOLUTO
				|| (this.modoUsuario == Utilidades.int_INDEFINIDO && sb.length() == 0)) {
			...
			if (this.modoUsuario == Fecha.MODO_MES_ACTUAL)
				sb.append("de este mes");
			else if (this.modoUsuario == Fecha.MODO_MES_SIGUIENTE)
				sb.append("del mes que viene");
			else if (this.mes != Utilidades.int_INDEFINIDO)
				sb.append(" de ").append(mesToString());
			if (this.modoUsuario == Fecha.MODO_ANIO_ACTUAL)
				sb.append("de este a�o");
			else if (this.modoUsuario == Fecha.MODO_ANIO_SIGUIENTE)
	 ...
	 *             Si la cadena no tiene alguno de los formatos antes indicados
	 */
	 ...
	 * @return boolean Devolver� true si esta fecha es m�s completa, esto es, tiene un n�mero
	 *         de campos definidos igual o mayor que la fecha pasada como par�metro y false e.o.c.
	 */
	...
	public static void main(String args[]) {
		/*
		 * Fecha d; d = Fecha.stringToFecha("9/11/01"); if(d != null)
		 * System.out.println("9/11/01 -> " + d.toString()); d = Fecha.stringToFecha("9/6/91"); if(d
		 * != null) System.out.println("9/6/91 -> " + d.toString()); d =
		 * Fecha.stringToFecha("19/10/01"); if(d != null) System.out.println("19/10/01 -> " +
		 * d.toString()); d = Fecha.stringToFecha("22/2/2001"); if(d != null)
		 * System.out.println("22/2/2001 -> " + d.toString()); d =
		 * Fecha.stringToFecha("19/11/1997"); if(d != null) System.out.println("19/11/1997 -> " +
		 * d.toString()); d = Fecha.stringToFecha("22/11"); if(d != null)
		 * System.out.println("22/11 -> " + d.toString()); d = Fecha.stringToFecha("01/10"); if(d !=
		 * null) System.out.println("01/10 -> " + d.toString());
		 */
	...
	/**
	 * M�todo que intenta completar una fecha primero con la lista de Fecha y luego mediante
	 * sentido com�n
	 * 
	 * @param listaSugerencias
	 */
	public void completar(Vector<Fecha> listaSugerencias) {
		...
		if (fechaCompletada.estaCompleta()) {
			...
			if (validacionFecha == Fecha.FECHA_VALIDA) {
				// La fecha es correcta
				if (!fechaCompletada.fechaEnCalendario())
					// La fecha es anterior a la fecha actual. Completamos la fecha con sentido
					// com�n
					this.completar();
				else
	 ...
	 * @return int
	 */
	private int factorComunDia(Vector<Fecha> listaSugerencias) {
		...
		while (i < listaSugerencias.size() && todosIguales) {
			if (diaAux != listaSugerencias.elementAt(i).dia)
	 ...
	 * @return int
	 */
	private int factorComunMes(Vector<Fecha> listaSugerencias) {
		int mes = Utilidades.int_INDEFINIDO;
		int mesAux = ((Fecha) listaSugerencias.elementAt(0)).mes;
		boolean todosIguales = true;
		int i = 0;
		while (i < listaSugerencias.size() && todosIguales) {
			if (mesAux != listaSugerencias.elementAt(i).mes)
				// Hemos encontrado uno que difiere del resto
				todosIguales = false;
			i++;
		}
	...
	/**
	 * M�todo que comprueba si todas las Fecha de la lista de entrada tienen en su fecha el mismo
	 * campo a�o Si todas tienen el mismo campo a�o devuelve dicho a�o y
	 * Utilidades.int_INDEFINIDO en caso contrario
	 * 
	 * @param listaSugerencias
	 * @return int
	 */
	private int factorComunAnio(Vector<Fecha> listaSugerencias) {
		int anio = Utilidades.int_INDEFINIDO;
		int anioAux = ((Fecha) listaSugerencias.elementAt(0)).anno;
		boolean todosIguales = true;
		int i = 0;
		while (i < listaSugerencias.size() && todosIguales) {
			if (anioAux != listaSugerencias.elementAt(i).anno)
				// Hemos encontrado uno que difiere del resto
				todosIguales = false;
			i++;
		}
