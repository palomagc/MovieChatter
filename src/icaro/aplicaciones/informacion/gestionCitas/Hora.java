 ...
 * 
 */
public class Hora implements Comparable<Object>, Cloneable, java.io.Serializable {
	 ...
	 *
	 * @param ms
	 *            n�mero de ms desde las 0 horas del 1 de enero de 1970
	...
	public static Hora getHoraFromMs(long ms) {
		...
		return new Hora(cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE),
	...
	public static Fecha getFechaFromMs(long ms) {
		...
		return new Fecha(cal.get(Calendar.DATE), cal.get(Calendar.MONTH) + 1,
	 ...
	 *
	 * @param hora
	 *            sustraendo de la operaci�n de resta
	...
	public boolean estaCompleta() {
		if (this.minutos != Utilidades.int_INDEFINIDO & this.hora != Utilidades.int_INDEFINIDO
				& this.segundos != Utilidades.int_INDEFINIDO
	...
	public void suma(Hora horaSum) {
		...
		if (this.hora >= 12)
			this.momentoDia = Hora.MOMENTO_DIA_PM;
		else
			this.momentoDia = Hora.MOMENTO_DIA_AM;
	...
	public static Hora string2Hora(String horaS) {
		// La hora en cadena tiene que tener el siguiente formato: "hh:mm:ss" o "hh:mm" o "h:mm" o
		// "h:mm:ss"
		Hora hora = new Hora();
		try {
			...
			hora = new Hora(Integer.parseInt(hora2S), Integer.parseInt(minutos),
		...
		try {
			if (obj != null) {
				...
				return (this.hora == h.hora && this.minutos == h.minutos
						&& this.segundos == h.segundos && this.momentoDia == h.momentoDia);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return boolean Devolver� true si esta hora es m�s completa, esto es, tiene un n�mero
	 *         de campos definidos igual o mayor que la hora pasada como par�metro y false e.o.c.
	 */
	...
	public void completar() {
		...
		// int primeraHoraCita =
		// Integer.parseInt(Configuracion.obtenerParametro("HORA_COMIENZO_CITAS"));
	 ...
	 * @param listaSugerencias
	 */
	public void completar(Vector<Hora> listaSugerencias) {
		...
		if (horaCompletada.estaCompleta()) {
			if (horaCompletada.horaEnHorarioLaboral()) {
				...
				// La fecha no est� dentro del horario laboral. Completamos la hora con sentido
				// com�n
				this.completar();
		} else {
	 ...
	 *
	 * @param horaNueva
	 *            hora con la que completar la hora actual
	 */
	 ...
	 * @return int
	 */
	private int factorComunHoras(Vector<Hora> listaSugerencias) {
		int horas = Utilidades.int_INDEFINIDO;
		int horasAux = listaSugerencias.elementAt(0).hora;
		boolean todosIguales = true;
		int i = 0;
		while (i < listaSugerencias.size() && todosIguales) {
			if (horasAux != listaSugerencias.elementAt(i).hora)
	...
	/**
	 * M�todo que comprueba si todas las Fecha de la lista de entrada tienen como momento del
	 * d�a el mismo momento Si todas tienen el mismo momento del d�a devuelve dicho momento y
	 * Utilidades.int_INDEFINIDO en caso contrario
	 * 
	 * @param listaSugerencias
	 * @return int
	 */
	private int factorComunMomentoDia(Vector<Hora> listaSugerencias) {
		int momento = Utilidades.int_INDEFINIDO;
		int momentoAux = listaSugerencias.elementAt(0).momentoDia;
		boolean todosIguales = true;
		int i = 0;
		while (i < listaSugerencias.size() && todosIguales) {
	...
	public boolean horaEnHorarioLaboral() {
		...
		int primeraHoraCita = 830;
		int ultimaHoraCita = 2030;
		if ((this.hora < primeraHoraCita) || (this.hora > ultimaHoraCita))
	...
	public static void main(String args[]) {
		...
		/*
		 * Hora h2 = new Hora(9, 54); Fecha f = new Fecha(1, 1, 1970);
		 * 
		 * System.out.println(Hora.getTimeStampUnix(f, h1));
		 * System.out.println(Hora.getTimeStampAnioMesDiaHora(f, h1));
		 * 
		 * if(h1.compareTo(h2)<0) System.out.println("menor"); else if(h1.compareTo(h2)>0)
		 * System.out.println("mayor"); else if(h1.compareTo(h2)==0) System.out.println("igual");
