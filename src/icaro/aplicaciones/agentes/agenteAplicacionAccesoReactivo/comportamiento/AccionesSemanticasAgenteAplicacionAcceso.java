...
public class AccionesSemanticasAgenteAplicacionAcceso extends AccionesSemanticasAgenteReactivo {
	...
	/**
	 * @uml.property name="visualizacion" Representa la interfaz de uso del recurso de
	 *               visualización
	 * @uml.associationEnd
	 */
	...
	/**
	 * Acción de inicialización de las actividades del agente. Se obtiene la interfaz del recurso
	 * de visualizacion y del recurso de persistencia para utilizarlos cuando haga falta se le
	 * ordena que muestre la ventana de acceso
	 */
	public void arranque() {
		try {
			visualizacion = (ItfUsoVisualizadorAcceso) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + "VisualizacionAcceso1");
			persistencia1 = (ItfUsoPersistenciaAccesoSimple) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + "Persistencia1");
	...
	/**
	 * Comprueba la validez de la informacion introducida por el usuario por medio del recurso de
	 * persistencia Se envía al automata un input "usuarioValido" o "usuarioNoValido" según el
	 * resultado de la validación para que se ejecute la transición correspondiente al estado en
	 * que se encuentra el agente. " infoUsuario.tomaUsuario(),infoUsuario.tomaPassword()" son los
	 * parametros que se le pasan a la accion asociada a la transición
	 *
	 * @param infoUsuario
	 *            contien la información introducida por el usuario y enviada por el recurso de
	 *            visualizacion
	 */
	public void valida(InfoAccesoSinValidar infoUsuario) {
		...
		try {
			if (ok) {
				this.informaraMiAutomata("usuarioValido", infoUsuario.tomaUsuario(),
						infoUsuario.tomaPassword());
			} else {
				this.informaraMiAutomata("usuarioNoValido", infoUsuario.tomaUsuario(),
						infoUsuario.tomaPassword());
			}
		} catch (Exception e) {
			try {
	...
	/**
	 * Esta accion es análoga a la anterior pero suponiendo que el agente esta en otro nodo En la
	...
	public void validaConRecRemoto(InfoAccesoSinValidar infoUsuario) {
		...
		try {
			...
			String identHostRecurso = config.getDescInstanciaRecursoAplicacion(nombreRec).getNodo()
					.getNombreUso();
	...
	/**
	 * Orden al recurso de visualización para que indique al usuario que ha accedido al sistema
	 * Despues se le comunica al gestor de agente el fin de actividades pidiendole la terminación
	 *
	 * @param username
	 * @param password
	 */
	...
	/**
	 * se ordena al visualizador que le indique al usuario que no ha accedido al sistema porque no
	 * esta registrado con esos dator
	 *
	 * @param username
	 * @param password
	 */
	public void mostrarUsuarioNoAccede(String username, String password) {
		...
		try {
			...
			visualizacion.mostrarMensajeError("Acceso Incorrecto",
					"El usuario " + dav.tomaUsuario() + " no ha accedido al sistema.");
		} catch (Exception ex) {
			try {
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"Ha habido un problema al abrir el visualizador de Acceso",
	...
	/**
	 * Acciones de terminación del agente: se cierran las ventana abiertas de visualización Se
	 * obtiene la interfaz propia de gestión y se ejecuta el métido terminar
	 */
	public void terminacion() {
		try {
			trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente, "Terminando agente: "
					+ NombresPredefinidos.NOMBRE_AGENTE_APLICACION + "Acceso1",
					InfoTraza.NivelTraza.debug));
			try {
				visualizacion.cerrarVisualizadorAcceso();
	...
	public void clasificaError() {
		...
		try {
			agenteAcceso = (ItfUsoAgenteReactivo) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + this.nombreAgente);
			agenteAcceso.aceptaEvento(new EventoRecAgte("errorIrrecuperable", this.nombreAgente,
					this.nombreAgente));
		} catch (Exception e) {
			try {
				trazas.aceptaNuevaTraza(new InfoTraza(
	...
	/**
	 * Se le pide al recurso de visualización que cierran las ventanas abiertas y se le envia al
	 * Gestor de agentes la petición de terminar todo que ha sido creada por el usuario y enviada a
	 * través del recurso de visualización. Se utiliza la clase comunicator que será la encargada
	 * de buscar la interfaz del agente receptor, crear un mensaje con la información a enviar y
	 * enviarla al agente.
	 */
		...
		try {
			...
			try {
				this.trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"Error al mandar el evento de terminar_todo", InfoTraza.NivelTraza.error));
			} catch (Exception e2) {
				e2.printStackTrace();
			}
			try {
