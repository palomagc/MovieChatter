...
public class AccionesSemanticasAgenteAplicacionAcceso extends AccionesSemanticasAgenteReactivo {
	...
	public void arranque() {
		...
		try {
			...
			visualizacion = (ItfUsoVisualizadorAcceso) obtenerIntfRecurso(
					identRecursoVisualizacionAcceso, NombresPredefinidos.ITF_USO);
			if (visualizacion != null) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"No se puede acceder al visualizador porque no se encuentra su interfaz de uso--"
								+ identRecursoVisualizacionAcceso, InfoTraza.NivelTraza.error));
		}
	...
	public void valida(InfoAccesoSinValidar infoUsuario) {
			...
			if (Persistencia1 != null) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"No se ha podido obtener la interfaz de uso del recurso"
								+ identRecursoPersistencia, InfoTraza.NivelTraza.error));
		}
		...
		try {
			...
			// itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte("usuarioNoValido",
	...
	public void validaConRecRemoto(InfoAccesoSinValidar infoUsuario) {
		...
		try {
			...
			if (ok) {
				agenteAcceso
						.aceptaEvento(new EventoRecAgte("usuarioValido", datosEnvio,
								this.nombreAgente, NombresPredefinidos.NOMBRE_AGENTE_APLICACION
										+ "Acceso"));
			} else {
				agenteAcceso.aceptaEvento(new EventoRecAgte("usuarioNoValido", datosEnvio,
	...
	public void terminacion() {
		...
		try {
			...
			// Buscar al Gestor de Agentes y si falla el GO y decirle que el agente de identificador
			// agxx ha terminado
	...
	public void clasificaError() {
		/*
		 * A partir de esta funci�n se debe decidir si el sistema se puede recuperar del error o
		 * no.En este caso la pol�tica es que todos los errores son cr�ticos.
		 */
		try {
			agenteAcceso = (ItfUsoAgenteReactivo) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + this.nombreAgente);
	...
	public void pedirTerminacionGestorAgentes() {
		try {
			 ...
			 * ("Itf_Uso_Gestor_Organizacion"); itfUsoGestorOrgan.aceptaEvento(new
			...
			itfUsoGestorAReportar = (ItfUsoAgenteReactivo) obtenerIntfAgente(identGestorAReportar,
					NombresPredefinidos.ITF_USO);
			if (itfUsoGestorAReportar == null) {
				...
				itfUsoGestorAReportar = (ItfUsoAgenteReactivo) itfUsoRepositorio
						.obtenerInterfaz(NombresPredefinidos.ITF_USO + identGestorAReportar);

			}
			this.itfUsoGestorAReportar.aceptaMensaje(new MensajeSimple(
					new InfoContEvtMsgAgteReactivo("peticion_terminar_todo"), this.nombreAgente,
	...
	public Object obtenerIntfAgente(String identAgente, String tipoInterfaz) {
		...
		try {
			...
			/*else if (resultadoBusqueda == null) { // No se ha conseguido encontrar la interfaz del
													// agente
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						"No se ha podido encontrar la interfaz del agente : " + identAgente,
						InfoTraza.NivelTraza.error));
			}*/
		} catch (Exception ex) {
			Logger.getLogger(AccionesSemanticasAgenteAplicacionAcceso.class.getName()).log(
					Level.SEVERE,
					"Se prujo un error al buscar   la interfaz del agente : " + identAgente, ex);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Se produjo un error al buscar   la interfaz del agente : " + identAgente,
					InfoTraza.NivelTraza.error));
		}
	...
	public Object obtenerIntfRecurso(String identRecurso, String tipoInterfaz) {
		...
		try {
			...
			/*else if (resultadoBusqueda == null) { // No se ha conseguido encontrar la interfaz del
													// agente
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						"No se ha podido encontrar la interfaz del agente : " + identRecurso,
						InfoTraza.NivelTraza.error));
			}*/
		} catch (Exception ex) {
			Logger.getLogger(AccionesSemanticasAgenteAplicacionAcceso.class.getName()).log(
					Level.SEVERE,
					"Se prujo un error al buscar   la interfaz del agente : " + identRecurso, ex);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Se produjo un error al buscar   la interfaz del agente : " + identRecurso,
					InfoTraza.NivelTraza.error));
		}
		return resultadoBusqueda;
	}
