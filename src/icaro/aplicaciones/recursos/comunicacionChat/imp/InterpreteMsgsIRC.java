...
public class InterpreteMsgsIRC {
	...
	public final void handleLine(String line) {
		...
		if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
			...
			if (request.startsWith("ACTION ")) {
				conectorIrc.onAction(sourceNick, sourceLogin, sourceHostname, target,
						request.substring(7));
				return;
			}
			...
			if (request.startsWith("PING ")) {
				conectorIrc.onPing(sourceNick, sourceLogin, sourceHostname, target,
						request.substring(5));
				return;
			}
	...
	protected void onPrivateMessage(String sender, String login, String hostname,
		...
		/**
		 * This method is called whenever an ACTION is sent from a user. E.g. such events generated
		 * by typing "/me goes shopping" in most IRC clients. The implementation of this method in
		 ...
		 */
		// Se envia la información al extrator semantico se traducen las
		...
		if (itfAgenteDialogo != null) {
			try {
				if (infoExtraida.size() == 0) {
					...
					// los elementos de la colección se meterán en el motor
				}
				itfAgenteDialogo.aceptaMensaje(mensajeAenviar);
	...
	private Notificacion interpretarAnotacion(String conttextoInterpretacion, Annotation anotacion) {
		...
		String msgNotif = conttextoInterpretacion
				.substring(posicionComienzoTexto, posicionFinTexto);
		notif.setTipoNotificacion(anotacion.getType());
