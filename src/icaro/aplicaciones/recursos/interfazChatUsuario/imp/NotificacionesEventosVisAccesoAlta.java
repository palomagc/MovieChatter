...
public class NotificacionesEventosVisAccesoAlta {
	...
	public NotificacionesEventosVisAccesoAlta(String identRecurso, String identAgteAreportar)
			throws Exception {
	...
	// public NotificacionesEventosVisAccesoAlta (ClaseGeneradoraVisualizacionAccesoAlta
	// generadoraVis)throws Exception {
	// // Obtenemos las informaciones que necesitamos de la clase generadora del recurso
	// generadoraVisualizador = generadoraVis;
	// trazas = NombresPredefinidos.RECURSO_TRAZAS_OBJ;
	...
	public void peticionCierreSistema() {
		EventoRecAgte eventoaEnviar = new EventoRecAgte("peticion_terminacion_usuario",
				...
				if (itfUsoAgenteaReportar == null)
					...
					NombresPredefinidos.RECURSO_TRAZAS_OBJ.aceptaNuevaTraza(new InfoTraza(
							...
							"No se puede enviar el evento pq la interfaz de uso del agente "
									+ identificadorAgenteaReportar + " es null ",
							InfoTraza.NivelTraza.error));
			} else {
				try {
	...
	public void PeticionDarAlta(String username, String password) throws Exception {
		...
		EventoRecAgte eventoaEnviar = new EventoRecAgte("peticion_alta", infoAutenticacion,
				identificadordeEsteRecurso, identificadorAgenteaReportar);
		enviarEvento(eventoaEnviar);
	}
	...
	public void PeticionDarAlta2(String username, String password) throws Exception {
		...
		// if (itfUsoAgenteaReportar == null){
		// Logger.getLogger(NotificacionesAgenteAccesoAlta.class.getName()).log(Level.SEVERE, null,
		// e);
		...
		if (itfUsoAgenteaReportar == null) {
			...
			try {
				...
				itfUsoAgenteaReportar.aceptaEvento(new EventoRecAgte("peticion_alta", datosEnvio,
						identificadordeEsteRecurso, identificadorAgenteaReportar));
	 ...
	 * 
	 * Object[] datosEnvio = new Object[]{username, password}; if (itfUsoAgenteaReportar == null)
	 * getInformacionAgenteaReportar(); try { itfUsoAgenteaReportar.aceptaEvento(new
	 * EventoRecAgte("peticion_alta"
	 * ,datosEnvio,identificadordeEsteRecurso,identificadorAgenteaReportar));
	 * 
	 * } catch (Exception e) {
	...
	public void enviarEvento(EventoRecAgte eventoaEnviar) {
		...
		if (eventoaEnviar == null)
			trazas.aceptaNuevaTraza(new InfoTraza(this.identificadordeEsteRecurso,
					"El evento a enviar en null " + eventoaEnviar + "al agente "
							+ identificadorAgenteaReportar, InfoTraza.NivelTraza.error));
		try {
			itfUsoAgenteaReportar.aceptaEvento(eventoaEnviar);
		} catch (Exception e) {
			// Logger.error("Ha habido un problema enviar un  evento al agente "+IdentAgenteReceptor);
			Logger.getLogger(NotificacionesEventosVisAccesoAlta.class.getName()).log(Level.SEVERE,
					null, e);
			NombresPredefinidos.RECURSO_TRAZAS_OBJ.aceptaNuevaTraza(new InfoTraza(
					this.identificadordeEsteRecurso,
