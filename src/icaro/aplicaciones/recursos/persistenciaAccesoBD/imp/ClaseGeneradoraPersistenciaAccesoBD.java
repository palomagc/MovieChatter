...
public class ClaseGeneradoraPersistenciaAccesoBD extends ImplRecursoSimple implements
	...
	public ClaseGeneradoraPersistenciaAccesoBD(String id) throws Exception {
		...
		super(id);
		/*
		 * try{ trazas = (ItfUsoRecursoTrazas)this.itfUsoRepositorioInterfaces.obtenerInterfaz(
		 * NombresPredefinidos.ITF_USO+NombresPredefinidos.RECURSO_TRAZAS); }catch(Exception e){
		...
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(this.getId(), "Creando el esquema " + id,
	...
	public boolean compruebaUsuario(String usuario, String password) throws ErrorEnRecursoException {
		try {
			...
			this.trazas.aceptaNuevaTraza(new InfoTraza(id, "Comprobando usuario " + usuario
					+ " Resultado consulta = " + resconsulta, InfoTraza.NivelTraza.debug));
			return resconsulta;
		} catch (Exception e) {
			e.printStackTrace();
			this.trazas.aceptaNuevaTraza(new InfoTraza(id, "Comprobando usuario " + usuario
					+ " Se ha producido un error: " + e.getMessage(), InfoTraza.NivelTraza.error));
			this.itfAutomata.transita("error");
			return false;
	...
	public boolean compruebaNombreUsuario(String usuario) throws ErrorEnRecursoException {
		trazas.aceptaNuevaTraza(new InfoTraza(this.getId(), "Comprobando nombre de usuario "
				+ usuario, InfoTraza.NivelTraza.debug));
	...
	public void insertaUsuario(String usuario, String password) throws ErrorEnRecursoException {
		trazas.aceptaNuevaTraza(new InfoTraza(this.getId(), "Insertando usuario " + usuario,
				InfoTraza.NivelTraza.debug));
	...
	@Override
	public void termina() {
		trazas.aceptaNuevaTraza(new InfoTraza(this.getId(), "Terminando recurso",
				InfoTraza.NivelTraza.debug));
