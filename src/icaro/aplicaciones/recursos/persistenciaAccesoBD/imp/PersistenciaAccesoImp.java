...
public class PersistenciaAccesoImp {
	...
	public static void crearEsquema(String idDescripcionIstanciaRecurso) throws Exception {
		try {
			if (obtenerParametrosBDPersistencia(idDescripcionIstanciaRecurso)) {
				 ...
				 * 
				 * rs = stmt.executeQuery( "CHECK TABLE "+AccesoBBDD.nombreBD+".USERS");
				 * while(rs.next()){ msg_text= rs.getString("Msg_text").trim(); }
				 * 
				 * //Solo se crean las tablas del script si no existen if (!msg_text.equals("OK")){
				 * System.out.println(
				 * "--------------Ejecuta script de creacion de las tablas bbdditems----------------"
				 * ); crearTablas(); //BDUtils BD=new BDUtils(); //BD.precargaRuben2();
				 * 
				 * 
				 * }
				 */
	...
	private static boolean obtenerParametrosBDPersistencia(String idDescripcionIstanciaRecurso)
			throws Exception {
	...
	private static boolean crearTablas() throws Exception {
		...
		/*
		 * Runtime r = Runtime.getRuntime(); Process p ; boolean esOK=false; try { String
		 * cmd="cmd /c  mysql --host="
		 * +AccesoBBDD.URL_CONEXION.substring(AccesoBBDD.URL_CONEXION.indexOf
		 * ("://")+3,AccesoBBDD.URL_CONEXION.lastIndexOf(":"))
		 * +" --port="+AccesoBBDD.URL_CONEXION.substring
		 * (AccesoBBDD.URL_CONEXION.lastIndexOf(":")+1,AccesoBBDD.URL_CONEXION.lastIndexOf(":")+5)
		 * +" --password="
		 * +AccesoBBDD.PASSWORD+" --user="+AccesoBBDD.LOGIN+" "+AccesoBBDD.nombreBD+" < "
		 * +AccesoBBDD.ejecutable; System.out.print("\n"+cmd+"\n\n"); p = r.exec(cmd); try {
		 ...
		 * System.err.println("Error al crear las tablas ("+AccesoBBDD
		 * .ejecutable+") de la base de datos "+AccesoBBDD.nombreBD); e.printStackTrace(); }
		 */
	...
	private static void conectar() throws ErrorEnRecursoException {
		...
		try {
			 ...
			 * System.out.println(urlname); conn = DriverManager.getConnection(urlname, LOGIN,
			 * PASSWORD); } catch (SQLException ex) {
			 * Logger.getLogger(AccesoBBDD.class.getName()).log(Level.SEVERE, null, ex); } }
			 */

			throw new ErrorEnRecursoException(
					"Ha habido un problema con la conexion con la base de datos " + URL_CONEXION
							+ "\nusuario " + LOGIN + "\npassword " + PASSWORD);
	...
	public boolean compruebaUsuario(String usuario, String password) throws ErrorEnRecursoException {
		...
		try {
			...
			resultado = query.executeQuery("SELECT * FROM " + PersistenciaAccesoImp.nombreBD
					+ ".tb_acceso U where U.user = '" + usuario + "' and U.password = '" + password
					+ "'");
			if (resultado.next())
				estado = true;
			else
