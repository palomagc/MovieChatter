...
public class AccionesSemanticasIniciador extends AccionesSemanticasAgenteReactivo {
		...
		try {
			...
			if (trazas == null) {
				...
				itfUsoRepositorio.registrarInterfaz(NombresPredefinidos.ITF_GESTION
						+ NombresPredefinidos.RECURSO_TRAZAS, trazas);
				// Guardamos el recurso de trazas y el repositorio de Itfs en la clase de nombres
				// predefinidos
	...
	public void crearRecursosNucleoOrganizacion() throws Exception {
		...
		// Intento crear los recursos en orden pero si se producen errores se capturan, se
		// visualizan y
		// se pude intentar dialogar con el usuario para que los corrija

		try {
			// Se crea el recurso de configuración y posteriormente se debería dar orden al
			// recurso
			...
			ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador", msgUsuario,
					InfoTraza.NivelTraza.error));
			// this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
			// "error_InterpretacionDescripcionOrganizacion",
			// NombresPredefinidos.NOMBRE_INICIADOR,
	...
	public String localizarComportamientoGestorInicial(String identGestor)
		...
		// Se obtiene el gestor inicial de la descripción de la organizacion. Puede ser un gestor
		// de organizacion o un gestor de nodo
		...
		try {
			...
			if (identGestor.startsWith(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)) {
				...
				// El gestor inicial debe ser un gestor de organización o un gestor de nodo y no se
				// ha definido ninguno
				ItfUsoRecTrazas
						.aceptaNuevaTraza(new InfoTraza(
								"Iniciador",
			...
			Logger.getLogger(AccionesSemanticasIniciador.class.getName()).log(Level.SEVERE, null,
					ex);
			ItfUsoRecTrazas
					.aceptaNuevaTraza(new InfoTraza(
							"Iniciador",
	...
	public void crearGestorInicial() throws Exception {
		...
		if (configuracionExterna.despliegueOrgEnUnSoloNodo())
			...
			if (thisHost.equalsIgnoreCase(identNodoGO)) // Esta en el nodo del GO se crea un Gestor
														// de Organizacion
				identGestorNodo = NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION;
			else
				identGestorNodo = NombresPredefinidos.NOMBRE_GESTOR_NODO; // Se crea un Gestor de
																			// Nodo
		}
		...
		if (!locComportamientoGestor.isEmpty()) {
			// Se crea el gestor y se registra en el registro RMI en el caso en que la organizacion
			// sea distribuida
			try {
				...
				itfgestGestorInicial = (ItfGestionAgenteReactivo) itfUsoRepositorio
						.obtenerInterfaz(NombresPredefinidos.ITF_GESTION + identGestorNodo);
	...
	/*public void arrancarGestorInicialyTerminar() throws Exception {
		...
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza("Iniciador", "Arrancando el Gestor Inicial . ",
					InfoTraza.NivelTraza.debug));
	...
	public void analizarInformeArranque(InformeArranqueGestor informeRecibido) {
		if (informeRecibido.getContenidoInforme().equals(
			...
			try {
				...
				Logger.getLogger(AccionesSemanticasIniciador.class.getName()).log(Level.SEVERE,
						null, ex);
			} catch (Throwable ex) {
				Logger.getLogger(AccionesSemanticasIniciador.class.getName()).log(Level.SEVERE,
						null, ex);
	...
	public void generarInformeErrorIrrecuperable() {
		 ...
		 * "error_en_arranque_gestores", NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
		 * NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
		 * this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte("informe_generado",
		 * nombreAgente, nombreAgente)); } catch (Exception e) { e.printStackTrace(); }
		 */
	}
	...
	public void crearAgente() {
		...
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"crearAgente():Este metodo no esta implementado", InfoTraza.NivelTraza.debug));
		} catch (Exception e2) {
			e2.printStackTrace();
		}
						...
						ItfUsoRecursoTrazas trazas = (ItfUsoRecursoTrazas) ClaseGeneradoraRepositorioInterfaces
								.instance().obtenerInterfaz(
										NombresPredefinidos.ITF_USO
												+ NombresPredefinidos.RECURSO_TRAZAS);
						trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente, "Agente " + nombre
	...
	public void terminarPorPropiaVoluntad() throws Exception, Throwable {
		...
		ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador", "Terminando el Iniciador.",
				InfoTraza.NivelTraza.debug));
		try {
			...
			itfUsoRepositorio.eliminarRegistroInterfaz(NombresPredefinidos.ITF_USO
					+ this.nombreAgente);
			itfUsoRepositorio.eliminarRegistroInterfaz(NombresPredefinidos.ITF_GESTION
					+ this.nombreAgente);
		...
		ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador", "Terminando.",
				InfoTraza.NivelTraza.debug));
		if (this.hebra != null)
	...
	public void comenzarTerminacionConfirmada() {
		...
		ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
				"Comenzando la terminacion de los recursos...", InfoTraza.NivelTraza.info));
